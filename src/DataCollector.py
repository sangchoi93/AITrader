...
class DataCollector:
    def __init__(self, logger, rh:Robinhood):
        self.logger = logger
        self.rh = rh
    ...
    def get_quote(self, symbol):
        ...
        quote = self.rh.quote_data(symbol)
    ...
    def convert_to_csv(self, symbol:str, l_data:list, prefix:str=None, suffix:str=None, filename:str=None):
        if l_data:
...
if __name__ == '__main__':
    ...
    while True:
        try:
            l_data += [dc.get_quote(symbol)]
            time.sleep(SLEEP_TIME)
            if len(l_data)%100 == 0:
                dc.convert_to_csv(symbol, l_data, 'dc')
                l_data = list()
